...
After wrangling, all data files contain the following five blocks of metrics in a single row per observation:
...
Pod-to-Pod traffic refers to traffic between Pods in the same cluster, and Pod-to-Server traffic refers to traffic from a Pod to an outside application.

The individual field names are derived from the first row in each data file.
...
The values for memory consumption (MB), CPU usage (Percentage), and
bandwidth (MBit/s) look fine.

For further analysis, we will inspect the key values a bit more
closely, starting with memory consumption. Looking at the idle values,
we get:
...
Validating this with the other measurements, we get a fairly consistent
picture:
...
Next, we will check CPU consumption, starting with idle values:
...
When we validate this with the other values, we get again fairly
consistent values across the measurements, but a clear differentiation
to the idle values:
...
This is the 2021 data set for Flannel, which we will read using R's
...
We'll extract the key metrics for further analysis:
...
This is the 2024 data set for Flannel, which we will read using R's
built-in read.csv function.
...
Compared to the previous years, we can see a significant increase in bandwidth.
